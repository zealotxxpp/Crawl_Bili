...
def get_cidtimestamp_DF(uid):
    ...
    sql = "SELECT cid, source_timestamp FROM video_danmus WHERE uid = %s GROUP BY cid, source_timestamp"
    cur.execute(sql % uid)
...
def video_danmus_insert(video_danmus_list):
...
def video_danmus_save(uid):
    ...
    while st < roundnum:
        ...
        video_danmus_insert(video_danmus_list)
        print('\r\n')
